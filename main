...
#define testThreadCount 4 //will make testThreadCount*2
#define testSize 8
...
void testThread1(MPMC_LFQ::LF_queue<std::atomic<char>, testSize>* q, st_recode* pushRec , st_recode* popRec , std::atomic<char>** testArray) {
	...
	q->pushWithRecycle(testArray[0], r);
	...
	for (count = 1; count < testThreadLoopCount; count++) {
		...
		pushStart = _Query_perf_counter();
		q->pushWithRecycle(testArray[count] , r);
		pushEnd = _Query_perf_counter();
		recode_update(pushRec, pushEnd - pushStart);
...
void testThread2(MPMC_LFQ::LF_queue<std::atomic<char>, testSize>* q, st_recode* pushRec, st_recode* popRec, std::atomic<char>** testArray) {
	long long popStart = 0;
	long long popEnd = 0;
	long long pushStart = 0;
	long long pushEnd = 0;
...
int main() {
	MPMC_LFQ::LF_queue<std::atomic<char>, testSize> q;
